...
class Store:
    ...
    def delete_engagement(self, eid: int) -> bool:
        """Delete an engagement and all associated tool runs and findings."""
        conn = get_conn()
        c = conn.cursor()
        try:
            ...
            # Delete tool runs
            c.execute("DELETE FROM tool_runs WHERE engagement_id=?", (eid,))
            # Delete engagement
